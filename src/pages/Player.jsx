    ...
    const highlightSearchTerm = useCallback((text, searchTerm) => {
        ...
        while (!matchFound && words.length >= starter) {
            ...
            if (start.length > 0) {
                ...
                // Check if the current word's length is 1 or 2 and it is part of the searchTerm
                if (words.slice(starter - 1).length === 1 && words[starter - 1].length <= 2) {
                    let singleOrDoubleCharWord = words[starter - 1];
                    if (normalizedSearchTerm.includes(singleOrDoubleCharWord)) {
                        start = kmp_matcher.kmp(normalizedText, singleOrDoubleCharWord);
                        if (start.length > 0) {
                            matchFound = true;
                            length = singleOrDoubleCharWord.length;
                        }
                    }
                    starter++;
                } else if (words.slice(starter - 1).length > 2) {
                    words.pop();
                } else {
                    starter++;
